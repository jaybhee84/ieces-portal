-- Upgrade existing contact saves to one selected guardian type-and-name pair.
...
comment on function public.save_advisory_contacts(jsonb) is
  'Updates the selected guardian type, guardian name, and contact number for learners managed by the signed-in Portal user.';
